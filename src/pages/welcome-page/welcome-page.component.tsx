...
import { useFilterSpring, useOpacitySpring } from '../../utils/spring';
...
export const WelcomePage = () => {
    ...
    const { freq, factor, scale, opacity } = useFilterSpring();
    const { spring: logoOpacitySpring } = useOpacitySpring(500, 2000);
    const { spring: textOpacitySpring } = useOpacitySpring(500, 2500);
    ...
    return (
        <div className={styles.welcomePage}>
			<div className={styles.logoContainer}>
                <animated.svg className={styles.svg} style={{ scale, opacity }} viewBox="0 0 72 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                        <filter id="water">
                    ...
                    <g filter="url(#water)">
                        ...
                        <animated.path style={logoOpacitySpring} d="M51.6485 23.469C51.6485 21.2048 53.518 19.3694 55.8242 19.3694C58.1305 19.3694 60 21.2048 60 23.469H51.6485Z" fill="#2E2F38"></animated.path>
                    </g>
                </animated.svg>
            </div>
            <animated.div style={textOpacitySpring} className={styles.title}>
                {t("homeTitle")}
            </animated.div>
            <animated.div style={textOpacitySpring} className={styles.blogButton} onClick={onReadClicked}>
                {t("homeButton")}
            </animated.div>
        </div>
    );
